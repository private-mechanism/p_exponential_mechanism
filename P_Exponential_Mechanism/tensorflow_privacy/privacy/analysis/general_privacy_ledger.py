...
p_exponnetial_sumquery_Entry=collections.namedtuple('p_exponnetial_sum_query',['exponents', 'dimensions', 'clipping_bound', 'beta'])
# Sample_query_Entry=collections.namedtuple('Sample_query_Entry',['population_size','sampling propability'])


sample_array=[]


#将数组表示的sample和query转化为开头定义的数据容器类型。其中sample:population_size, selection_probability, num_queries；
def format_ledger(sample_array, query_array):
  ...
  for sample in sample_array:
    ...
    sample_pos += 1
  return samples
#formatted后的ledger长的样子是
#[SampleEntry(population_size=10.0, selection_probability=0.1, queries=[GaussianSumQueryEntry(l2_norm_bound=10.0, noise_stddev=0.0)]), SampleEntry(population_size=20.0, selection_probability=0.2, queries=[GaussianSumQueryEntry(l2_norm_bound=10.0, noise_stddev=0.0)])]


#账本类中包含的几个方法：do_record_将query记录到buffer中，结束一次sample query的记录，从buffer中读取ledger
class PrivacyLedger(object):
  ...
  ########################################在buffer中进行记录必须考虑读写锁的问题。
    ...
    """
    #这里是在将query记录到ledger上。顺序是：self._query_count + 1，然后在buffer里面append[self._sample_count, l2_norm_bound, noise_stddev]
...
#结束一次sample过程的记录（采样过程+添加噪声）。sample_count+1，query_count置为0
...
#首先从buffer里读出sample和query数组；然后调用文件开头定义的format函数转化为entry形式
