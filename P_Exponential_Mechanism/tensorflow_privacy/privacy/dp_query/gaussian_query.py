  ...
  def get_noised_result(self, sample_state, global_state):
    ...
    if LooseVersion(tf.__version__) < LooseVersion('2.0.0'):
      def add_noise(v):
        ...
        # gn=tf.global_norm([v])
        # print(gn)
        # gn=gn.eval(session=sess)
        # print('#################################################################################################',gn)
        # print('######################################################################################################',tf.global_norm([v]))
        noise=tf.random.normal(tf.shape(input=v), stddev=global_state.stddev)
        # print('##################################################################################',tf.norm(noise,ord=2).eval(session=sess))
        return v + noise
    else:
    ...
    gn=tf.global_norm([sample_state[7]])
    # sess = tf.Session()
    # gn =tf.convert_to_tensor(np.array([tf.norm(sample_state[i]) for i in range(len(sample_state))]))
    if self._ledger:
      dependencies = [
          self._ledger.record_sum_query(
              global_state.l2_norm_clip, gn)
      ]
    else:
      dependencies = []
