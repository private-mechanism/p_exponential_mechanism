...
"""Training a CNN on cifar10 with differentially private SGD optimizer."""
...
flags.DEFINE_float('exponents', 1.2,
                   'exponents of the p-exponential distribution where the noises are sampled')
...
def cnn_model_fn(features, labels, mode):
   ...
   learing_rate = tf.train.exponential_decay(0.15, global_step, 100, 0.96, staircase=False)
...
def main(unused_argv):
  ...
  for epoch in range(1, FLAGS.epochs + 1):
    cifar10_classifier.train(input_fn=train_input_fn, steps=steps_per_epoch)
